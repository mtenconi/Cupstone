...
# Different segments have different habits and needs, therefore, after the segmentation process marketers will be able to develop different strategies to target each specific segment.
# In this way, the mall will be able to focus on particular target groups so as to provide the best experience for them.
...
# Download the dataset from the website 'https://www.kaggle.com/vjchoudhary7/customer-segmentation-tutorial-in-python'; 
# the file is downloaded in the 'downloads' folder.
# Import the file in R (everybody needs to indicate the correct path according with their pc):
...
# Each row represents one costumer identified by the customer ID
head(data)

# Missing values:
which(is.na(data))   # no missing values are present
...
# group and subgroups needed for the following analyses:
...
# C. Plots
# Count plots of gender and density plot of age distribution, by gender
Male_percentage <- length(data$Gender[data$Gender == "Male"])/length(data$Gender)*100
Female_percentage <- length(data$Gender[data$Gender == "Female"])/length(data$Gender)*100
Male_percentage
Female_percentage
...
# Gender and age are usually good indicators for distinguishing target groups. 
# Here men are slightly more represented than women. There is a difference also in the activity of the two groups (males & females): 
# both males and females are highly active between the ages 25-35 ca., however, women have a second pick of activity between the ages 45-50 ca. while after the age of 45, men's' activity declines.
...
b3 <- data %>% ggplot(aes(x = Annual.Income_20, y = Spending.Score..1.100., fill = age_20))  + 
  ...
  ggtitle("Spending score stratified by annual income")
...
Annual_income_020 <- length(Annual.Income_20[Annual.Income_20 == "[0,20)"])/length(data$Gender)*100
Annual_income_2040 <- length(Annual.Income_20[Annual.Income_20 == "[20,40)"])/length(data$Gender)*100
Annual_income_4060 <- length(Annual.Income_20[Annual.Income_20 == "[40,60)"])/length(data$Gender)*100
Annual_income_6080 <- length(Annual.Income_20[Annual.Income_20 == "[60,80)"])/length(data$Gender)*100
Annual_income_80100 <- length(Annual.Income_20[Annual.Income_20 == "[80,100)"])/length(data$Gender)*100
Annual_income_100120 <- length(Annual.Income_20[Annual.Income_20 == "[100,120)"])/length(data$Gender)*100
Annual_income_120140 <- length(Annual.Income_20[Annual.Income_20 == "[120,140)"])/length(data$Gender)*100
Annual_income_020
Annual_income_2040
Annual_income_4060
Annual_income_6080
Annual_income_80100
Annual_income_100120
Annual_income_120140 
# The two groups (males and female) seem to have a similar trend and there are not great anomalies, with exception of a few outliers in the males' spending score distribution (image F1).
# However, some interesting observations emerge when performing the boxplots of spending score stratified by annual income, and grouped by gender and age:
# in general some important differences between male and female behavior are evident, with exception for customers with an annual income between 40-80k $,
# while customers younger than 40 years old usually have higher spending score, with exception of customers with an annual income between 40-60k $, representing the 23% of the entire customers
...
# PCA works best with numerical data, therefore, the variable gender is transformed in a numerical variable
# Data need to be standardized to make variables comparable
...
# I obtain 4 principal components (pc1-pc4). 
...
# new data set obtained by the pca, with pc1, pc2, pc3, pc4, age_20, Annual.Income_20, Spending.Score_25, Gender:
...
pca1b <- pca_df %>% 
  ...
  labs( x = "pc 1 (49.15% explained var.) ", y = "pc 2 (30.30% explained var.)")
...
pca2b <- pca_df %>% 
  ...
  geom_encircle(data = subset(pca_df, age_20 == "[40,60)"), size =4, s_shape= 1, expand=0.05, spread = 0, alpha = 0.2) +
  geom_encircle(data = subset(pca_df, age_20 == "[60,80)"), size =4, s_shape= 1, expand=0.05, spread = 0, alpha = 0.2) +
  labs( x = "pc 1 (49.15% explained var.) ", y = "pc 3 (19.56% explained var.)")
...
pca3b <- pca_df %>% 
  ...
  labs( x = "pc 2 (30.30% explained var.) ", y = "pc 3 (19.56% explained var.)")
...
# the two groups (Male and Female) are completely overlapped, therefore they do not provide significant information.



# 4.2. K-means clustering: 
# K-means clustering is an unsupervised machine learning algorithm for partitioning a given data set into a set of k groups (clusters), where k represents the number of groups pre-specified by the analyst. 
# It classifies objects in multiple clusters, such that objects within the same cluster are as similar as possible (high intra-class similarity), whereas objects from different clusters are as dissimilar as possible (low inter-class similarity). 
...
# According to PCA results, Annual Income and spending score are the two more informative variable, therefore, I perform k-means clustering using Annual Income and Spending Score:
set.seed(123)
...
# Choose the number of clusters, based on the elbow method (where there is a bend/knee in the plot)
...
# Annual Income vs Spending score scatter plot showing the distribution of the 5 clusters:
...
# extract the clusters and do some descriptive statistics at the cluster level:
data %>%
...
# extract the clusters and do some descriptive statistics at the cluster and gender level:
data %>%
  mutate(Cluster = final2$cluster) %>%
...
# Original data set with indication of the clusters:
...
# Despite through some data exploration and data visualization (chapter 3) it seemed to emerge some interesting observations and important differences between male and female behavior
# from the analysis of the clusters grouped by gender, it does not result any important difference and the two groups (female and male) can be treated together:
data %>%
  mutate(Cluster = final2$cluster) %>%
  group_by(Cluster, Gender) %>% select(Cluster, Age, Annual.Income..k.., Spending.Score..1.100.) %>%
  summarise_all(c(avg="mean")) %>% knitr::kable()
